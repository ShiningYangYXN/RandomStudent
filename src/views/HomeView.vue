...
<template>
  <el-container>
    <el-main>
      <el-config-provider v-if="nameList.length === 0" size="large">
        <el-result icon="warning" title="配置名单" sub-title="请先前往设置配置名单,然后再开始抽选"
          ><template #extra>
            <RouterLink to="/settings"><el-button type="primary">好的</el-button></RouterLink>
          </template>
        </el-result>
      </el-config-provider>
      <template v-else>
        ...
        <el-row justify="center">
          <Transition>
            ...
            >
            <el-button type="danger" @click="Stop()" v-else style="font-size: 3rem; height: 5rem"
